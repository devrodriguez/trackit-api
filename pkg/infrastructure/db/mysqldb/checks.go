...
func (c *ChecksAdapter) QueryCheckedEmployee(employee entity.Employee, company entity.Company, date string) ([]entity.CheckQuery, error) {
	...
	rows, err := c.dbConn.
		...
		Where("checks.employee_id = ? and companies.id = ? and checks.date = ?", employee.ID, company.ID, date).
		Rows()
...
func (c *ChecksAdapter) QueryChecksEmployee(employee entity.Employee) ([]entity.CheckQuery, error) {
	var checks []entity.CheckQuery
	selectQuery := `checks.id as check_id, checks.date, checks.address, checks.hour,
					check_types.id as check_type_id, check_types.name as check_type_name, 
					employees.id as employee_id, employees.name as employee_name, employees.email,
